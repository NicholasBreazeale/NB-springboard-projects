...
describe("GET /companies", function () {
  ...
  test("ok for minEmployees and maxEmployees filter", async function () {
  ...
  test("bad request if minEmployees is greater than maxEmployees", async function () {
  ...
  test("bad request if invalid query key", async function () {
    const resp = await request(app).get(`/companies?foo=bar`);
    expect(resp.statusCode).toEqual(400);
  });
